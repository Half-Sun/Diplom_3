from conftest import driver, random_user, restore_password_page
import allure
...
class TestMyAccountPage:
    ...
    def test_go_to_restore_password_page(self, restore_password_page):
        assert restore_password_page.get_restore_password_email_field_text() == "Email"

    @allure.title("Test input data in email field")
    def test_input_data_in_email_field(self, restore_password_page):
        restore_password_page.input_data_in_restore_password_email_field_and_click()
        assert restore_password_page.get_text_from_element(
            MyAccountLocators.RESTORE_PASSWORD_REMEMBER_PASSWORD_LINK) == "Войти"

    @allure.title("Test click on show password, make it shine")
    def test_click_on_show_password_make_it_shine(self, restore_password_page):
        restore_password_page.input_data_in_restore_password_email_field_and_click()
        restore_password_page.input_data_in_restore_password_form_field_and_click_on_eye()
        new_password_input = restore_password_page.find_element_and_wait(
            MyAccountLocators.RESTORE_PASSWORD_NEW_PASSWORD_INPUT_SHOWN)
    ...
    def test_go_to_my_account_profile(self, driver, random_user):
        ...
        assert my_account_page.get_text_from_element(
            MyAccountLocators.MY_ACCOUNT_TEXT) == "В этом разделе вы можете изменить свои персональные данные"
    ...
    def test_my_account_logout(self, driver, random_user):
        ...
        assert my_account_page.get_text_from_element(MyAccountLocators.MY_ACCOUNT_ENTER_BUTTON) == "Войти"
